...
#define LSOne(S) (S & (-S))
//least significant one - for x take x complement, plus 1, then logical and operation of x & ~x+1 gives LSone
// For an unsigned type, -x is equivalent to TYPE_MAX - x + 1, which is equivalent to ~x + 1. It's easier to see why x &= ~x + 1 works.
